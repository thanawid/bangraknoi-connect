...
const els = {
  ...
  cityDock:          $('#cityDock'),
  cityDockTitle:     $('#cityDockTitle'),
  cityDockDescription: $('#cityDockDescription'),
  cityDockList:      $('#cityDockList'),
  cityDockAction:    $('#cityDockAction'),
  citySpeech:        $('#citySpeech'),
};


// ── Genesis City Stage ───────────────────────────────────────────────────────
function renderCityDock(id) {
  const cat = getZone(id);
  if (!cat || !els.cityDock) return;

  $$('.city-hotspot, .city-chip').forEach((el) => {
    el.classList.toggle('active', el.dataset.category === id);
  });

  els.cityDock.dataset.category = id;
  els.cityDockTitle.textContent = cat.label;
  els.cityDockDescription.textContent = cat.description;
  if (els.citySpeech) els.citySpeech.textContent = `ถึงแล้วครับ — ${cat.label}`;

  const items = (cat.items || []).slice(0, 3);
  els.cityDockList.replaceChildren();
  items.forEach((item) => {
    const a = document.createElement('a');
    a.className = 'city-dock-link';
    a.href = item.href || '#';
    ...
    els.cityDockList.append(a);
  });

  els.cityDockAction.onclick = () => openDrawer(id);
}

function setupCityStage() {
  if (!els.cityDock) return;
  $$('.city-hotspot, .city-chip').forEach((btn) => {
    btn.addEventListener('click', () => renderCityDock(btn.dataset.category));
  });
  renderCityDock('service');
}
...
function setupNavigation() {
  ...
  const sections = ['city','search','categories','news','about']
